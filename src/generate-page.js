...
function generateCards(roster) {
  let cards = []
  for(let i = 0; i < roster.length; i++) {
    const rosterArray = roster[i];
    switch(rosterArray.getRole()) {
      case 'Manager':
        const manager = new Manager(rosterArray.id, rosterArray.name, rosterArray.email, rosterArray.officeNumber);
        cards.push(generateManagerCard(manager));
        break;
      case 'Engineer':
        const engineer = new Engineer(rosterArray.id, rosterArray.name, rosterArray.email, rosterArray.github);
        cards.push(generateEngineerCard(engineer));
        break;
      case 'Intern':
        const intern = new Intern(rosterArray.id, rosterArray.name, rosterArray.email, rosterArray.school);
